...
class AporteManager:
    ...
    def total(self, user_id):
        ...
        return len(aporte.timeline)

    def construir_lotes_copia(
        self,
        user_id,
        max_items=100,
    ):
        """Construye lotes ordenados sin cortar un álbum."""
        aporte = self.get_active(user_id)
        ...
        for unidad in unidades:
            ...
            message_ids = unidad["message_ids"]

            if len(message_ids) > max_items:
                raise ValueError(
                    "Un álbum supera el límite permitido por Telegram"
                )
    ...
    def terminar_intento_envio(
        ...
        if not completado:
            # No se permiten archivos nuevos después de un fallo porque
            # el progreso del envío parcial ya está guardado.
    ...
    def get_delivery_progress(
        ...
        return aporte.delivery_progress.setdefault(
            destination_id,
            {
                # Primero se envía toda la media. Después se envía la
                # ficha final con el comentario.
                "batches_sent": 0,
                "summary_parts_sent": 0,
                "completed": False,
            },
        )
    ...
    def limpiar(self, user_id):
        aporte_id = self.active.pop(
            user_id,
            None,
        )

        if aporte_id:
            self.aportes[user_id].pop(
                aporte_id,
                None,
            )
